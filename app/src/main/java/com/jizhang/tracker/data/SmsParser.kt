...
object SmsParser {
    ...
    fun parse(sms: SmsMessage, triggerKeywords: Set<String> = emptySet()): ParseResult? {
        val body = sms.body.trim()

        val isWechat = body.contains("微信")
        val isAlipay = body.contains("支付宝")
        val isBank = detectBank(body)
        val isTriggered = isWechat || isAlipay || isBank || triggerKeywords.any { body.contains(it) }
        ...
        // 匹配自定义关键词但无法识别来源，按手动处理
        val type = if (forceExpense) TransactionType.EXPENSE else TransactionType.EXPENSE
        return ParseResult(amount, type, TransactionSource.MANUAL)
    }

    private fun hasNegativeAmount(text: String): Boolean = AmountParser.hasNegative(text)

    private fun extractAmount(text: String): Double? =
        AmountParser.firstOf(text, AmountParser.CURRENCY, AmountParser.RMB_PREFIX, AmountParser.YUAN)
