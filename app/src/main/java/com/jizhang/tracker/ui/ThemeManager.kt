...
const val MATERIAL_YOU_THEME_INDEX = -2
...
data class CustomThemeConfig(
    ...
    companion object {
        ...
        fun toJson(config: CustomThemeConfig): String =
            moshi.adapter(CustomThemeConfig::class.java).toJson(config)

        fun fromJson(json: String): CustomThemeConfig {
            return try {
                moshi.adapter(CustomThemeConfig::class.java).fromJson(json) ?: CustomThemeConfig()
            } catch (_: Exception) {
                CustomThemeConfig()
            }
        }
